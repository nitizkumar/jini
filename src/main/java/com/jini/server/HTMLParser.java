...
public class HTMLParser
{
    ...
    throws Exception
  {
    ...
    for (Element element : allElements)
    {
      ...
      {
        if ((!element.hasAttr("type")) || (element.attr("type").equals("text"))) {
          if (element.hasAttr("length"))
          {
            String attr = element.attr("length");
            int len = Integer.parseInt(attr);
            element.replaceWith(new TextNode(Lorem.getInstance().word(len), ""));
          }
          else
          {
            element.replaceWith(new TextNode(Lorem.getInstance().full(), ""));
          }
        }
        if ((element.hasAttr("type")) && (element.attr("type").equals("name"))) {
          element.replaceWith(new TextNode(Lorem.getInstance().name(), ""));
        }
        if ((element.hasAttr("type")) && (element.attr("type").equals("firstname"))) {
          element.replaceWith(new TextNode(Lorem.getInstance().firstName(), ""));
        }
        if ((element.hasAttr("type")) && (element.attr("type").equals("lastname"))) {
          element.replaceWith(new TextNode(Lorem.getInstance().lastName(), ""));
        }
        if ((element.hasAttr("type")) && (element.attr("type").equals("image")))
        {
          ...
          imgTag.attr("src", Lorem.getInstance().getImageURL(width, height));
  ...
  protected String resolvePartials()
    ...
    throws Exception
  {
    ...
    FileUtils.writeToFile(new File("/media/9C1CA03D1CA01474/projects/solver-middleman/build/index.html"), parse);
  }
}
